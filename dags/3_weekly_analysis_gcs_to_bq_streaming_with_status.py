...
SOURCE_PREFIX = f"analysis_final/clean_batch_results/{RUN_ID}"
GCS_STATUS_BLOB = "analysis_final/status.json"
...
def is_target_jsonl_object(object_name: str, source_prefix: str) -> bool:
...
def list_source_jsonl_objects(gcs_hook: GCSHook, source_prefix: str) -> List[str]:
    ...
    jsonl_objects = sorted(
        ...
        if is_target_jsonl_object(object_name, source_prefix)
    )
...
def read_pipeline_status(gcs_hook: GCSHook) -> Dict[str, Any]:
    """
    讀取 DAG2 寫在 GCS 的 status.json。
    DAG3 只在 status = DONE 時才會匯入 BigQuery。
    """
    try:
        raw_data = gcs_hook.download(bucket_name=BUCKET_NAME, object_name=GCS_STATUS_BLOB)
    except Exception as exc:
        logging.warning(
            "Status file gs://%s/%s not found or cannot be read: %s",
            BUCKET_NAME,
            GCS_STATUS_BLOB,
            exc,
        )
        return {
            "status": "RUNNING",
            "message": "status.json 不存在，視為上游尚未完成。",
        }
    ...
    try:
        ...
        logging.warning("Status file is not valid JSON, skip this run: %s", exc)
        return {
            "status": "RUNNING",
            "message": "status.json 格式錯誤，視為上游尚未完成。",
        }
...
with DAG(
    ...
    def sync_reviews_analysis() -> None:
        ...
        status_payload = read_pipeline_status(gcs_hook)
        upstream_status = str(status_payload.get("status", "")).upper()

        if upstream_status != "DONE":
            logging.info(
                "Skip this run because upstream status is %s. message=%s",
                upstream_status,
                status_payload.get("message", ""),
            )
            return

        if status_payload.get("ready_to_import") is False:
            logging.info(
                "Skip this run because status is DONE but ready_to_import=false. message=%s",
                status_payload.get("message", ""),
            )
            write_pipeline_status(
                ...
                status_payload,
                status="IMPORTED",
                message="DAG3 已確認本次沒有需要匯入的 JSONL，略過 BigQuery 寫入。",
                extra={"imported_rows": 0, "imported_reviews": 0, "imported_files": 0},
            )
            return

        status_run_id = status_payload.get("run_id")
        if status_run_id and status_run_id != RUN_ID:
            logging.info(
                "Skip this run because status.json run_id=%s does not match today's RUN_ID=%s.",
                status_run_id,
                RUN_ID,
            )
            return

        # 固定只匯入本日 weekly_run_YYYYMMDD 的 clean JSONL。
        # status.json 只負責判斷 DAG2 是否已完成，不覆蓋 SOURCE_PREFIX，避免誤匯入其他日期資料。
        source_prefix = SOURCE_PREFIX

        jsonl_objects = list_source_jsonl_objects(gcs_hook, source_prefix)

        if not jsonl_objects:
            logging.warning(
                "Status is DONE but no target JSONL files found under gs://%s/%s. Skip without failing.",
                BUCKET_NAME,
                source_prefix,
            )
            return
        ...
        write_pipeline_status(
            ...
            status_payload,
            status="IMPORTED",
            message="DAG3 已完成 JSONL 匯入 BigQuery，避免後續 10 分鐘排程重複匯入同一批資料。",
            extra={
                "imported_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
                "imported_files": total_files,
                "imported_reviews": total_reviews,
                "imported_rows": total_rows,
                "target_table": TARGET_TABLE,
                "imported_run_id": RUN_ID,
                "imported_source_prefix": SOURCE_PREFIX,
            },
        )
